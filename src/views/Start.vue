  ...
  <el-container style="overflow-x: hidden" v-if="flag">
    ...
    <el-main>
      ...
      >
        ...
        <template v-if="!(questionIndex === testLength)">
          <div class="title">
            <h3>{{ titleName }}</h3>
          </div>
          <div class="main">
            <div class="progress">
              <el-progress
                ...
                :percentage="value"
              ></el-progress>
              <span>{{ questionIndex }}/{{ testLength }}</span>
            ...
            <div class="question">
              ...
              <h3>
                {{ questionIndex + 1 }}、{{
                  testQuestionAll[questionIndex]?.title
                }}
              </h3>
              <!-- 试题选项 -->
              <div
                class="option"
                v-for="(item, i) in testQuestionAll[questionIndex]?.anwsers"
                :key="item.id"
...
export default {
  ...
  methods: {
    ...
    next(i, dom, answer, answerScore) {
      ...
      */
      option.forEach((item) => (item.style.pointerEvents = "none"));
      // 获取当前选择的试题标题
      const itemTitle = this.testQuestionAll[this.questionIndex]?.title;
      ...
      setTimeout(() => {
        // 试题进度索引++
        this.questionIndex++;
        if (this.questionIndex === this.testLength) {
          this.calculation();
          // 向数据库中添加答题数据
          let data = {
            userId: this.userId, // 用户id
            questionId: this.rid, // 获取用户名称
            pname: this.name, // 获取用户名称
            questionContent: this.testData, // 当前用户选择的分值
            score: this.totalValue, // 试题总分值
            picturebox: this.picturebox, // 试题图片
            title: this.titleName, // 试题标题
          };
          this.loading = true;
          addUserResult(data).then((res) => {
            if (res.code === 200) {
              this.$router.push({
                path: `/result/${this.rid}`,
              });
            }
          });
        }
        ...
        // 计算进度条数值
        this.value = this.questionIndex * average;
      }, 1000);
    },
    // 返回进度条总数的平均值
    average() {
